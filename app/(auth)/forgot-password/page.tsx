...
import { ArrowLeft, Mail, Loader2, Eye, EyeOff, CheckCircle2, RotateCcw } from 'lucide-react'
...
export default function ForgotPasswordPage() {
  ...
  const handleOtpChange = (i: number, value: string) => {
    ...
    const next = [...otp]; next[i] = value; setOtp(next); setOtpError('')
  ...
  const handleResend = async () => {
    setResending(true); setOtpError(''); setOtp(['', '', '', '', '', ''])
    inputRefs.current[0]?.focus()
    await fetch('/api/auth/forgot-password', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email }),
    })
    setCooldown(COOLDOWN); setResending(false)
  }
  ...
  const onPasswordSubmit = async (data: PasswordForm) => {
    ...
    const res    = await fetch('/api/auth/reset-password', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, code: otp.join(''), newPassword: data.password }),
    })
    const result = await res.json() as { error?: string }
    if (!res.ok) { setResetError(result.error ?? 'Failed to reset password'); return }
    setStep('done')
  ...
  if (step === 'otp') {
    ...
    return (
      <div className="glass rounded-2xl border border-border/60 p-8 text-center">
        ...
        <div className="flex justify-center gap-2 mb-5" onPaste={handleOtpPaste}>
          {otp.map((digit, i) => (
            <input key={i} ref={el => { inputRefs.current[i] = el }}
              ...
              value={digit} disabled={otpLoading}
              ...
              className={[
                ...
                digit ? 'border-brand ring-2 ring-brand/20' : 'border-border focus:border-brand focus:ring-2 focus:ring-brand/20',
        ...
        <Button onClick={() => { if (filled === 6) verifyCode(otp.join('')) }}
          disabled={filled < 6 || otpLoading}
          className="w-full bg-brand text-white hover:bg-brand/90 border-0 mb-5">
          {otpLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Verifying…</> : 'Verify code'}
        </Button>
    ...
    <div className="glass rounded-2xl border border-border/60 p-8">
      ...
      <form onSubmit={pwForm.handleSubmit(onPasswordSubmit)} className="space-y-4">
          ...
          {pw && (
            <div className="flex gap-4 mt-1.5">
              {Object.entries(checks).map(([k, ok]) => (
                <span key={k} className={`text-[11px] ${ok ? 'text-green-400' : 'text-muted-foreground'}`}>
                  {ok ? '✓' : '·'}{' '}{k === 'length' ? '8+ chars' : k === 'upper' ? 'Uppercase' : 'Number'}
                </span>
              ))}
            </div>
          )}
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <Input id="cf" type={showCf ? 'text' : 'password'}
  ...
  return (
    <div className="glass rounded-2xl border border-border/60 p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground">Enter your email and we&apos;ll send a 6-digit code.</p>
      </div>
      ...
      <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4">
        <div className="space-y-1.5">
          ...
          <Input id="email" type="email" placeholder="you@company.com"
            autoComplete="email" {...emailForm.register('email')} />
